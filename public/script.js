const vertShaderScript = `#version 300 es
...
const fragShaderScript = `#version 300 es
...
let devicePixelRatioUniform;
let scaleFactorUniform;
let timeUniform;
let deviceOrientationUniform;
...
function setupCanvas() {
  ...
  webGlContext.shaderSource(vertexShader, vertShaderScript);
  ...
  webGlContext.shaderSource(fragmentShader, fragShaderScript);
  ...
  deviceOrientationUniform = webGlContext.getUniformLocation(
    program,
    "uDeviceOrientation"
  );
  devicePixelRatioUniform = webGlContext.getUniformLocation(
    program,
    "uDevicePixelRatio"
  );
  scaleFactorUniform = webGlContext.getUniformLocation(program, "uScaleFactor");
  timeUniform = webGlContext.getUniformLocation(program, "uTime");
...
function animationUpdate() {
  ...
  const width = canvas.getBoundingClientRect().width * window.devicePixelRatio;
  const height =
  ...
  // Update the uniforms.
  {
    webGlContext.uniform4f(deviceOrientationUniform, 0, 0, 0, 1);
  }

  webGlContext.uniform1f(devicePixelRatioUniform, window.devicePixelRatio);
  webGlContext.uniform2f(
    scaleFactorUniform,
    width / window.devicePixelRatio,
    height / window.devicePixelRatio
  );
  webGlContext.uniform1f(timeUniform, (Date.now() - timestamp) * 0.001);
